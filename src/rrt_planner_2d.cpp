...
RRTPlanner2D::Plan2DT RRTPlanner2D::CreatePath(const unsigned int& start_mx,
  ...
  rrt_star_->InitializeStateSpace(collision_checker_->GetSizeX(),
                                  collision_checker_->GetSizeY(), 1);
...
RRTPlanner2D::PlanT RRTPlanner2D::ProcessPath(
  ...
  std::for_each(path.begin(), path.end(), [&](const auto& coordinates) {
    pose.pose =
        GetWorldCoordinates(coordinates.x, coordinates.y, collision_checker_);
    plan.push_back(pose);
  });
